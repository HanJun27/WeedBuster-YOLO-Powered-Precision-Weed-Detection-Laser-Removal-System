...
class CalibDiffuseNode(Node):
    ...
    def sample_dark(self):
        """连续采 N 帧 RGB 红通道 + IR 灰度的均值，作为暗电流。"""
        rs = []
        nirs = []
        t0 = time.time()
    ...
    def sample_gray(self):
        ...
        while len(rs) < GRAY_FRAME_COUNT and time.time() < deadline:
            ...
            if roi_rgb.size == 0 or roi_ir.size == 0:
                time.sleep(0.05)
                continue
            rs.append(float(np.mean(roi_rgb[:, :, 2])))
            nirs.append(float(np.mean(roi_ir)))
            time.sleep(1.0 / CAM_FPS)

        if len(rs) < 5:
            return False, "采样帧不足"

        dn_r_raw = float(np.mean(rs))
        dn_nir_raw = float(np.mean(nirs))
        # 减暗电流
        dn_r_corr = max(1.0, dn_r_raw - self.dark_R)
        dn_nir_corr = max(1.0, dn_nir_raw - self.dark_NIR)
        ...
        self.dn_r_gray = dn_r_raw
        self.dn_nir_gray = dn_nir_raw
        self.k_active = dn_r_corr / dn_nir_corr

        self.get_logger().info(
            f"✅ 灰卡: R_raw={dn_r_raw:.2f} R'={dn_r_corr:.2f}, "
            f"NIR_raw={dn_nir_raw:.2f} NIR'={dn_nir_corr:.2f}, "
